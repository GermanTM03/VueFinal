...
export default {
  async obtenerUsuarios(): Promise<IUser[]> {
    try {
      const response = await axios.get<{ mensaje: string, response: IUser[] }>(`${API_URL}/Usuarios`);
  ...
  async obtenerActividades(): Promise<IActividad[]> { // Corregido: eliminado 'function' de la definición de la función
    try {
      const response = await axios.get<{ mensaje: string, response: IActividad[] }>(`${API_URL}/Actividades`);
      console.log(response.data); // Verifica qué está devolviendo la API
      return response.data.response; // Devuelve response.data.response
    } catch (error) {
      console.error('No se pudieron recuperar las actividades:', error);
